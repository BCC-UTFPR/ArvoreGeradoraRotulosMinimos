...
def exists_not_connected(label_list):
	if len(label_list) > 0:
		return True
	else:
		return False
...
class Chromosome:	
	def chromosome(self,graph,size):
		...
		while(exists_not_connected(node_list)):
			if len(label_list) <= 0:
				print 'Não foi possível gerar um grafo a partir desta lista de rótulos'
				return None
...
class Crossover:
	...
	def crossover(self,S,graph,size): # Recebe a concatenação do cromossomo a e do b em S
